...
class CausalAttention(nn.Module):
    def __init__(self, seq_len:int=512, embed_dim:int=64, head_size:int=32, n_head:int=8, n_query_groups:int=4, sliding_window_size=4):
        super().__init__()
        ...
        """
            ...
            └────────────────────────┘  └───────────────────────┘  └─────────────────────────┘
        """
        sliding_window_bias = torch.ones_like(self.mask).tril(diagonal=-sliding_window_size)
        sliding_window_bias.masked_fill_(sliding_window_bias.bool(), float("-inf"))
        self.mask += sliding_window_bias
        print(self.mask)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:            
        ...
        print(self.num_queries_per_group)
...
class MultiQueryAttention(nn.Module):
    ...
    """
    def __init__(self, seq_len:int=512, head_size:int = 32, embed_dim: int = 64, n_head:int=8, n_query_groups:int=1) -> None:
        super().__init__()
        self.n_head = n_head
        self.embed_dim = embed_dim
        self.head_size = head_size
        ...
        self.mqa = CausalAttention(seq_len, embed_dim, head_size, n_head, n_query_groups)
...
if __name__=="__main__":
    mha = MultiQueryAttention(12,32,64,8)
    input = torch.randn([4,12,64])
    output = mha(input)
    print(output.shape)