...
class MyBatchNorm1d(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        if self.training:
            ...
            sample_var = torch.var(
                x, dim=0, unbiased=False
            )  # do not apply bessel's correction
            if self.track_running_stats:
                self.update_running_mean_and_var(
                    sample_mean, sample_var, x.shape[0]
                )  # use these stats to update running stats
            if self.affine:
                return (
                    (x - sample_mean) / torch.sqrt(sample_var + self.eps)
    ...
    def forward_test(self, x):
        if self.affine:
            return (
                (x - self.running_mean) / torch.sqrt(self.running_var + self.eps)
            ) * self.gamma + self.beta
        else:
    ...
    def update_running_mean_and_var(self, mean, var, num_samples):
        ...
        self.running_var = (
            ...
            + self.momentum * var * correction_factor
        )  # apply correction for variance
...
if __name__ == "__main__":
    ...
    train_input = torch.exp(torch.randn([batch_size, num_features])) + torch.sin(
    ...
    # test time
    test_input = torch.exp(torch.randn([batch_size, num_features])) + torch.sin(
        torch.randn([batch_size, num_features])
    )
    mybatchnorm.eval()
    batchnorm.eval()
